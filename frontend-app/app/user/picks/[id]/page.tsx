...
export default async function PickDetailPage({
  ...
  const { pb: contextPB, league } = await getLeagueContext();
  const pb = contextPB ?? (await getPB());

  let pick;
  try {
    pick = await pb.collection("picks").getOne(id, {
      expand: "game, league_team, week_record",
    });
  } catch {
    notFound();
  }
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-2xl space-y-6 p-6">
        ...
        <Link className="inline-block text-sm underline" href={league?.slug ? `/user/leagues/${league.slug}/picks` : "/user/picks"}>
          Back to picks
        </Link>
      </main>
    </>
