...
export default function LeaguePageHeader({ league, leagues }: { league: { name: string; slug: string }; leagues: { name: string; slug: string }[] }) {
  ...
  const selectLeague = (slug: string) => {
    document.cookie = `pickems_league=${encodeURIComponent(slug)}; path=/; max-age=31536000; samesite=lax`;
    router.push(`/user/leagues/${slug}/dashboard`);
  };
    ...
    <div className="mx-auto flex w-full max-w-6xl items-center px-4 pt-6 sm:px-6 lg:px-8">
      <div className="flex min-w-0 items-center gap-3">
        ...
        <div className="min-w-0">
          ...
          <Select value={league.slug} onValueChange={selectLeague}>
